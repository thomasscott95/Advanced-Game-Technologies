...
PhysicsSystem::PhysicsSystem(GameWorld& g) : gameWorld(g)	{
	...
	SetGravity(Vector3(0.0f, -75.8f, 0.0f));
}
...
void PhysicsSystem::ImpulseResolveCollision(GameObject& a, GameObject& b, CollisionDetection::ContactPoint& p) const {
	...
	if (a.GetName() == "Speed" && b.GetName() == "Player") {
		physB->SetLinearVelocity(Vector3(-157, 0, 0));
	}

	if (a.GetName() == "Speed1" && b.GetName() == "Player") {
		physB->SetLinearVelocity(Vector3(-15, 0, 0));
	}
	if (a.GetName() == "Speed2" && b.GetName() == "Player") {
		physB->SetLinearVelocity(Vector3(15, 0, 0));
	}

	if (a.GetName() == "Ice1" && b.GetName() == "Player") {
		physB->SetLinearVelocity(Vector3(35, 0, 12));
		physB->SetAngularVelocity(Vector3(0, 0, 0));
	}
	if (a.GetName() == "Ice2" && b.GetName() == "Player") {
		physB->SetLinearVelocity(Vector3(35, 0, 0));
		physB->SetAngularVelocity(Vector3(0, 0, 0));
	}
	if (a.GetName() == "Ice3" && b.GetName() == "Player") {
		physB->SetLinearVelocity(Vector3(35, 0, -12));
		physB->SetAngularVelocity(Vector3(0, 0, 0));
	}
	...
	Vector3 friction = (-contactVelocity - (p.normal * (Vector3::Dot(contactVelocity, p.normal)))).Normalised();
	...
	Vector3 frictionInertiaA = Vector3::Cross(physA->GetInertiaTensor() * Vector3::Cross(relativeA, friction), relativeA);
	Vector3 frictionInertiaB = Vector3::Cross(physB->GetInertiaTensor() * Vector3::Cross(relativeB, friction), relativeB);
	...
	float fAngularEffect = Vector3::Dot(frictionInertiaA + frictionInertiaB, friction);
	...
	float jFriction = (-fFriction * Vector3::Dot(contactVelocity, friction)) / (totalMass + fAngularEffect);
	Vector3 fullFriction = friction * jFriction;
